...
const MovieDetails = () => {
  ...
  return (
    <View className="bg-primary flex-1">
      ...
      >
        ...
        <View className="flex-col items-start justify-center mt-5 px-5">
          ...
          <View className="flex flex-row justify-between w-1/2">
            <MovieInfo
              label="Budget"
              value={`$${movie?.budget ?? 0 / 1_000_000}`}
            />
            <MovieInfo
              label="Revenue"
      ...
      <TouchableOpacity
        className="absolute bottom-5 left-0 right-0 mx-5 flex flex-row items-center bg-accent rounded-lg py-3.5 justify-center z-50"
        onPress={() => router.back()}
      >
        <Image
          source={icons.arrow}
